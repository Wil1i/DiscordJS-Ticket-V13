async function fetchMessage(message, numberOfMessages) {
    ...
    const messageSize = channelMessages.size
        ...
        await fs.readFile(require('path').join(__dirname, 'template.html'), 'utf8', async function(err, data) {
            if(data) {
                await fs.writeFile(require('path').join(__dirname, 'index.html'), data, async function(err) {                if(err) return console.log(err) 
                    ...
                    messagecount.appendChild(document.createTextNode(`Fetched Messages: ${messageSize}`))
                    ...
                    await fs.appendFile(require('path').join(__dirname, 'index.html'), info.outerHTML, async function(err) {
                        ...
                        msgs.forEach(async msg => {
                            ...
                            if(msg.content.startsWith("```")) {
                                ...
                                const messageContentFilter = msg.content.split(" ").filter(message => message.startsWith("<@"))
                                
                                for(const mention of messageContentFilter){
                                    const userID = mention.replace("<", "").replace("@", "").replace("&", "").replace(">", "")
                                    let replaceWith = mention
                                    if(mention.includes("&")){
                                        const findRole = message.guild.roles.cache.get(userID)
                                        if(findRole){
                                            replaceWith = `@${findRole.name}`
                                        }
                                    }else{
                                        const findUser = message.guild.members.cache.get(userID)
                                        if(findUser){
                                            replaceWith = `@${findUser.user.username}`
                                        }
                                    }
                                    messageContent = messageContent.replace(mention, replaceWith)
                                }
                                let msgNode = document.createElement('span');
                                let textNode = document.createTextNode(messageContent);
